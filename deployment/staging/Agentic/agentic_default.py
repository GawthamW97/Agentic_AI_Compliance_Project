...
def retrieve_candidates(product_desc, collection, k=10, timing=None):
    ...
    if timing is not None:
        timing["embedding_sec"] = round(embed_end - embed_start, 3)
        timing["chroma_query_sec"] = round(query_end - embed_end, 3)
...
def agentic_classify(product_desc, collection, k):
    ...
    candidates = retrieve_candidates(product_desc, collection, k, timing=timing)

    reasoning_start = time.perf_counter()
    reasoning = reasoning_module(product_desc, candidates)
    timing["reasoning_sec"] = round(time.perf_counter() - reasoning_start, 3)
    ...
    if not is_valid:
        ...
        timing["reflection_sec"] = round(time.perf_counter() - reflection_start, 3)
        is_valid, validation_msg = validate_classification(reasoning, candidates)

    timing["total_sec"] = round(time.perf_counter() - overall_start, 3)
